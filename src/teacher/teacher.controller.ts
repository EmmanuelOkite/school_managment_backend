...
export class TeacherController {
  ...
  @ApiOperation({
    ...
    description:
      'Creates a new teacher record. The Teacher ID / Employee Number is ' +
      'auto-generated by the server (format: T-{year of joining}-{6-digit sequence}) ' +
      'and must not be supplied by the client. ' +
      'A login account (username/password) is only created when createLoginAccount is true; ' +
      'password is hashed before storage and never returned in any response. ' +
      'email must be unique, and username must be unique when a login account is requested.',
  })
  ...
  @ApiResponse({
    ...
    schema: { example: { message: 'Teacher "T-2026-000001" deleted successfully.' } },
  })
  @ApiResponse({ status: 404, description: 'Teacher not found.' })
